...
class PlayerScoreSchema(BaseSchema):
    id = fields.Int(required=True, description="プレイヤーID")
    name = fields.Str(required=True, description="プレイヤー名")
    games_played = fields.Int(dump_default=0, description="参加ゲーム数")

    total_score = fields.Float(dump_default=0.0, description="合計スコア")


class TournamentExportSchema(BaseSchema):
    tournament = fields.Dict(required=True, description="大会情報")
    players = fields.List(
...
class GroupSummarySchema(BaseSchema):
    group = fields.Dict(required=True, description="グループ情報")
...
class PlayerScoreMapSchema(BaseSchema):
    ...
    total = fields.Float(dump_default=0.0, description="合計スコア（全テーブルの合計）")
    converted_total = fields.Float(
        dump_default=0.0, description="換算スコア（合計スコア × レート）"
    )


class TableInfoSchema(Schema):
    id = fields.Int(required=True, description="テーブル（卓）のID")
    name = fields.Str(required=True, description="テーブル（卓）の名前")
    type = fields.Enum(
        TableTypeEnum, required=True, description="テーブル（卓）のタイプ"
    )
...
class TournamentScoreMapSchema(BaseSchema):
    ...
    tournament_id = fields.Int(required=True, description="大会ID")
    tables = fields.List(
    ...
    rate = fields.Float(dump_default=1.0, description="換算レート")
...
class GroupPlayerStatSchema(BaseSchema):
    ...
    total_score = fields.Float(
        dump_default=0.0, description="得点合計（チップ・換算含まない）"
    )
    total_balance = fields.Float(
        dump_default=0.0, description="収支（チップ・換算含む）"
    )
...
class GroupPlayerStatsSchema(BaseSchema):
    ...
    period = fields.Dict(
        keys=fields.Str(), values=fields.Str(), description="集計期間", dump_default={}
    )
