...
from app.models import (
    AccessLevel, Game, Group, GroupCreationToken, IdempotencyRecord, Player,
    Score, ShareLink, Table, TablePlayer, TableTypeEnum, Tournament,
    TournamentPlayer,
)
...
def _replay(scope, key, payload):
    if key and len(key) > 255:
        raise V2Error(400, "VALIDATION_ERROR", "Idempotency-Key must be at most 255 characters")
    if not key:
        return None
    ...
    if record.request_hash != _hash(payload):
        raise V2Error(409, "IDEMPOTENCY_KEY_REUSED", "Idempotency-Key was used with a different request")
...
def _remember(scope, key, payload, body, status):
    ...
    if key:
        db.session.add(IdempotencyRecord(
            scope=scope, idempotency_key=key, request_hash=_hash(payload),
...
def create_tournament_with_tables(group_key, payload, idempotency_key=None):
    ...
    if any(not item.get("name") for item in initial_tables) or len(client_ids) != len(set(client_ids)):
        raise V2Error(400, "VALIDATION_ERROR", "Initial table names and unique client_id values are required")
    try:
        tournament = Tournament(
            group_id=group.id, name=payload["name"], description=payload.get("description"),
            rate=payload.get("rate", 1.0), created_by=group.created_by,
        ...
        for item in initial_tables:
            try:
                ...
                raise V2Error(400, "VALIDATION_ERROR", "type must be NORMAL or CHIP") from exc
            table = Table(
                tournament_id=tournament.id, name=item["name"], type=table_type,
                created_by=tournament.created_by, created_at=datetime.now(timezone.utc),
            )
...
def batch_add_participants(tournament_key, payload, idempotency_key=None):
    ...
    if not ids or None in ids or len(ids) != len(set(ids)):
        raise V2Error(400, "VALIDATION_ERROR", "participants must contain unique player_id values")
    players = Player.query.filter(Player.id.in_(ids)).all()
    if len(players) != len(ids) or any(p.group_id != tournament.group_id for p in players):
    ...
    try:
        ...
        for table in tables:
            ...
            propagated.append({"table_id": table.id, "type": table.type.value, "added_count": len(table_added)})
        body = {
            "tournament_id": tournament.id,
            "participants": [_player(next(p for p in players if p.id == pid)) for pid in ids],
            "added_count": len(added_ids), "already_registered_count": len(existing.intersection(ids)),
...
def delete_participant(tournament_key, player_id, propagate_to=None, idempotency_key=None):
    ...
    if replay:
        return replay
    _, tournament = _require(tournament_key, "tournament", Tournament, AccessLevel.EDIT)
    participant = TournamentPlayer.query.filter_by(tournament_id=tournament.id, player_id=player_id).first()
    if not participant:
    ...
    scored = []
    if table_ids:
        scored = [row[0] for row in db.session.query(Game.table_id).join(Score).filter(
            Game.table_id.in_(table_ids), Score.player_id == player_id
        ).distinct().all()]
    if scored:
        raise V2Error(409, "PARTICIPANT_HAS_SCORES", "Participant has scores in one or more chip tables", {"table_ids": scored})
    try:
        if table_ids:
            TablePlayer.query.filter(TablePlayer.table_id.in_(table_ids), TablePlayer.player_id == player_id).delete(synchronize_session=False)
        db.session.delete(participant)
        body = {"deleted": {"tournament_id": tournament.id, "player_id": player_id, "propagated_table_ids": table_ids}}
        _remember(scope, idempotency_key, payload, body, 200)
        db.session.commit()
        return body, 200
    except Exception:
...
def cascade_delete_table(table_key, idempotency_key=None):
    ...
    game_ids = [row[0] for row in db.session.query(Game.id).filter_by(table_id=table.id).all()]
    score_count = Score.query.filter(Score.game_id.in_(game_ids)).count() if game_ids else 0
    game_count = len(game_ids)
    participant_count = TablePlayer.query.filter_by(table_id=table.id).count()
    table_id = table.id
    try:
        if game_ids:
            Score.query.filter(Score.game_id.in_(game_ids)).delete(synchronize_session=False)
        ...
        ShareLink.query.filter_by(resource_type="table", resource_id=table.id).delete(synchronize_session=False)
        db.session.delete(table)
        body = {"deleted": {"table_id": table_id, "game_count": game_count, "score_count": score_count, "participant_count": participant_count}}
        _remember(scope, idempotency_key, payload, body, 200)
        db.session.commit()
        return body, 200
    except Exception:
...
def batch_get_groups(payload):
    ...
    if None in keys or len(keys) != len(set(keys)):
        raise V2Error(400, "VALIDATION_ERROR", "group_key values must be present and unique")
    links = {link.short_key: link for link in ShareLink.query.filter(ShareLink.short_key.in_(keys)).all()}
    results = []
    for item in items:
...
def batch_group_status(payload):
    ...
    if None in tokens or len(tokens) != len(set(tokens)):
        raise V2Error(400, "VALIDATION_ERROR", "token values must be present and unique")
    records = {record.token: record for record in GroupCreationToken.query.filter(GroupCreationToken.token.in_(tokens)).all()}
    ...
    for item in items:
        ...
        if not record:
            ...
            if expires < now:
                ...
                owner = ShareLink.query.filter_by(resource_type="group", resource_id=record.group_id, access_level=AccessLevel.OWNER).first()
                if owner:
