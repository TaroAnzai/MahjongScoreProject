...
class TestTournamentEndpoints:
    def test_create_tournament_requires_group_edit(
        self, client, db_session, create_group
    ):
    ...
    def test_create_tournament_with_view_link_forbidden(
        self, client, db_session, create_group
    ):
    ...
    def test_delete_tournament_requires_group_edit(
        self, client, db_session, create_group
    ):
        _group_data, links = create_group()
        res = _create_tournament(client, links[AccessLevel.EDIT.value])
    ...
    def test_delete_tournament_removes_zero_score_chip_table(
        ...
        table_id = table["id"]
        table_links = {
            link["access_level"]: link["short_key"] for link in table["table_links"]
        }
        ...
        create_game(
            table_edit_key,
            players,
            scores=[{"player_id": player["id"], "score": 0} for player in players],
        )

        deleted = client.delete(f"/api/tournaments/{tournament_edit_key}")
    ...
    def test_get_tournaments_by_group(self, client, db_session, create_group):
        ...
        # まずグループ内に大会を2件作成
        res1 = _create_tournament(
            client, links[AccessLevel.EDIT.value], name="Autumn Cup"
        )
        res2 = _create_tournament(
            client, links[AccessLevel.EDIT.value], name="Winter Cup"
        )
        ...
        # GET: 大会一覧取得
        res_list = client.get(
            f"/api/groups/{links[AccessLevel.VIEW.value]}/tournaments"
        )
        ...
        assert any(
            "group_key" in m for m in res_404.get_json()["errors"]["json"]["message"]
        )
    ...
    def test_get_tables_by_tournament(self, client, db_session, create_group):
        ...
        tournament_links = {
            link["access_level"]: link["short_key"]
            for link in tournament_data.get_json()["tournament_links"]
        }
        # --- 卓を2件登録 ---
        for i in range(1, 3):
            res = client.post(
        ...
        # --- GET: 一覧取得 ---
        res_list = client.get(
            f"/api/tournaments/{tournament_links[AccessLevel.VIEW.value]}/tables"
        )
        assert res_list.status_code == 200
        ...
        assert res_404.status_code == 404
        assert (
            "共有リンクが無効です。" in res_404.get_json()["errors"]["json"]["message"]
        )
