...
class TestGameEndpoints:
    def test_create_game_requires_table_edit(
    ...
    def test_create_game_with_view_link_forbidden(
        self, client, db_session, setup_full_tournament, create_game
    ):
        data = setup_full_tournament(client)
        table_links = data["table_links"]
    ...
    def test_delete_game_requires_table_edit(
        self, client, db_session, setup_full_tournament
    ):
        data = setup_full_tournament(client)
        game = data["game"]
        ...
        forbidden = client.delete(
            f"/api/tables/{table_links[AccessLevel.VIEW.value]}/games/{game['id']}"
        )
        assert forbidden.status_code == 403

        allowed = client.delete(
            f"/api/tables/{table_links[AccessLevel.EDIT.value]}/games/{game['id']}"
        )
        assert allowed.status_code == 200
