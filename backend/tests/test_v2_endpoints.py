...
def add_links(db_session, resource_type, resource_id, prefix):
    ...
    for level in (AccessLevel.OWNER, AccessLevel.EDIT, AccessLevel.VIEW):
        link = ShareLink(
            short_key=f"{prefix}-{level.value.lower()}",
...
def create_tournament(client, group_key, payload=None, headers=None):
    return client.post(
        ...
        json=payload or {"name": "大会"},
        headers=headers or {},
    )


def test_create_tournament_with_initial_tables_is_atomic_and_idempotent(
    client, db_session, v2_group
):
    ...
    assert {
        link["access_level"] for link in body["tournament"]["tournament_links"]
    } == {"VIEW", "EDIT"}
    created_table = body["created_tables"][0]["table"]
    assert {link["access_level"] for link in created_table["table_links"]} == {
        "VIEW",
        "EDIT",
    }
...
def test_create_tournament_rolls_back_everything_on_invalid_table(
    client, db_session, v2_group
):
    _, links = v2_group
...
def test_omitting_initial_tables_preserves_single_create_behavior(
    client, db_session, v2_group
):
    _, links = v2_group
...
def v2_tournament(db_session, v2_group):
    ...
    players = [
        Player(group_id=group.id, name=name) for name in ("田中", "鈴木", "佐藤")
    ]
    ...
    chip = Table(
        ...
        name="チップ",
        type=TableTypeEnum.CHIP,
        created_by="test",
    )
    normal = Table(
        tournament_id=tournament.id,
        name="1卓",
        type=TableTypeEnum.NORMAL,
        created_by="test",
    )
...
def test_batch_add_is_idempotent_and_propagates_only_to_chip(
    client, db_session, v2_tournament
):
...
def test_delete_participant_conflicts_when_chip_score_exists(
    client, db_session, v2_tournament
):
...
def test_cascade_delete_table_reports_and_removes_children(
    client, db_session, v2_tournament
):
    ...
    response = client.delete(
        f"/api/v2/tables/{chip_links['EDIT']}",
        headers={"Idempotency-Key": "delete-chip"},
    )
...
def test_dashboards_return_available_players_and_games(
    ...
    group_links, tournament, tournament_links = (
        v2_tournament[1],
        v2_tournament[2],
        v2_tournament[3],
    )
    normal, normal_links, players = (
        v2_tournament[6],
        v2_tournament[7],
        v2_tournament[-1],
    )
    ...
    group_body = client.get(
        f"/api/v2/groups/{group_links['VIEW']}/dashboard"
    ).get_json()
    tournament_body = client.get(
        f"/api/v2/tournaments/{tournament_links['VIEW']}/dashboard"
    ).get_json()
    table_body = client.get(
        f"/api/v2/tables/{normal_links['VIEW']}/dashboard"
    ).get_json()
    ...
    assert [p["id"] for p in tournament_body["available_group_players"]] == [
        players[2].id
    ]
    ...
    assert {
        link["access_level"]
        for link in group_body["tournaments"][0]["tournament_links"]
    } == {"VIEW"}
    assert {
        link["access_level"]
        for link in tournament_body["tournament"]["tournament_links"]
    } == {"VIEW"}
    assert {
        link["access_level"]
        for link in tournament_edit_body["tournament"]["tournament_links"]
    } == {"VIEW", "EDIT"}
    deprecated_fields = {"view_link", "edit_link", "owner_link", "parent_group_link"}
    assert {
        link["access_level"] for link in tournament_body["tables"][0]["table_links"]
    } == {"VIEW"}
    assert {
        link["access_level"]
        for link in tournament_body["score_map"]["tables"][0]["table_links"]
    } == {"VIEW"}
    assert {
        link["access_level"]
        for link in tournament_edit_body["tables"][0]["table_links"]
    } == {"VIEW", "EDIT"}
    assert {link["access_level"] for link in table_body["table"]["table_links"]} == {
        "VIEW"
    }
    assert {
        link["access_level"] for link in table_edit_body["table"]["table_links"]
    } == {"VIEW", "EDIT"}
    removed_table_fields = {"view_link", "edit_link", "owner_link"}
    assert not removed_table_fields.intersection(tournament_body["tables"][0])
    assert not removed_table_fields.intersection(
        tournament_body["score_map"]["tables"][0]
    )
    ...
    assert [p["id"] for p in table_body["available_tournament_players"]] == [
        players[1].id
    ]
...
def test_batch_group_creation_status_enum(client, db_session, v2_group):
    ...
    db_session.commit()
    response = client.post(
        "/api/v2/groups/request-link/status:batch",
        json={
            "items": [
                {"client_id": "0", "token": "pending"},
                {"client_id": "1", "token": "ready"},
                {"client_id": "2", "token": "expired"},
                {"client_id": "3", "token": "unknown"},
            ]
        },
    )
    assert response.status_code == 200
...
def test_participant_delete_always_propagates_to_chip(
    client, db_session, v2_tournament
):
    ...
    assert (
        TablePlayer.query.filter_by(table_id=chip.id, player_id=player.id).count() == 0
    )
    assert (
        TablePlayer.query.filter_by(table_id=normal.id, player_id=player.id).count()
        == 1
    )
...
def test_v2_openapi_contract_is_complete(client, test_app):
    ...
    expected_success = {
        ("/api/v2/groups/{group_key}/tournaments", "post"): (
            "201",
            "TournamentCreateV2Response",
        ),
        ("/api/v2/groups:batch-get", "post"): ("200", "GroupBatchGetResponse"),
        ("/api/v2/groups/{group_key}/dashboard", "get"): (
            "200",
            "GroupDashboardResponse",
        ),
        ("/api/v2/groups/request-link/status:batch", "post"): (
            "200",
            "StatusBatchResponse",
        ),
        ("/api/v2/tournaments/{tournament_key}/participants:batch-add", "post"): (
            "200",
            "ParticipantBatchAddResponse",
        ),
        ("/api/v2/tournaments/{tournament_key}/participants/{player_id}", "delete"): (
            "200",
            "ParticipantDeleteResponse",
        ),
        ("/api/v2/tournaments/{tournament_key}/dashboard", "get"): (
            "200",
            "TournamentDashboardResponse",
        ),
        ("/api/v2/tables/{table_key}", "delete"): ("200", "TableDeleteResponse"),
        ("/api/v2/tables/{table_key}/dashboard", "get"): (
            "200",
    ...
    mutation_operations = [
        ...
        paths["/api/v2/tournaments/{tournament_key}/participants/{player_id}"][
            "delete"
        ],
    ...
    for schema_name, field_descriptions in v2_schema._FIELD_DESCRIPTIONS.items():
        ...
        for field_name, expected_description in field_descriptions.items():
            assert (
                schema_class._declared_fields[field_name].metadata["description"]
    ...
    batch_add_properties = spec["components"]["schemas"]["ParticipantBatchAdd"][
        "properties"
    ]
    ...
    propagated_ids = spec["components"]["schemas"]["ParticipantDeletedResource"][
        "properties"
    ]["propagated_table_ids"]
